...
use vfs::{
    DirectoryBatch, DirectorySink, EntryCapabilities, FileEntry, FileKind, ListOptions, ProviderId,
    ProviderCapabilities, ResourceUri, VfsError, VfsProvider,
};
...
impl LocalFsProvider {
    ...
    fn stat_blocking(uri: ResourceUri) -> Result<FileEntry, VfsError> {
        ...
        let metadata = fs::symlink_metadata(&path).map_err(|error| Self::map_io_error(&uri, error))?;
...
fn list_blocking(
    ...
    let read_dir = fs::read_dir(&path).map_err(|error| LocalFsProvider::map_io_error(&uri, error))?;
...
mod tests {
    use super::*;
    use std::io::Write;
    use vfs::{ListSessionId, VfsProvider};
