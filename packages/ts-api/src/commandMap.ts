export const commandMap: Record<string, string> = {
  "app.get_info": "app_get_info",
  "fs.stat": "fs_stat",
  "fs.read_text_file": "fs_read_text_file",
  "fs.read_file_range": "fs_read_file_range",
  "fs.read_file_as_data_uri": "fs_read_file_as_data_uri",
  "fs.read_image_as_data_uri": "fs_read_image_as_data_uri",
  "fs.write_text_file": "fs_write_text_file",
  "fs.compute_hash": "fs_compute_hash",
  "fs.open_terminal": "fs_open_terminal",
  "fs.list_start": "fs_list_start",
  "fs.list_directories": "fs_list_directories",
  "fs.list_archive": "fs_list_archive",
  "fs.standard_locations": "fs_standard_locations",
  "fs.discover_volumes": "fs_discover_volumes",
  "fs.eject_volume": "fs_eject_volume",
  "fs.diff_text": "fs_diff_text",
  "fs.open_default": "fs_open_default",
  "fs.reveal": "fs_reveal",
  "fs.properties": "fs_properties",
  "git.discover": "git_discover",
  "git.statusForDirectory": "git_status_for_directory",
  "fs.folder_size": "fs_folder_size",
  "fs.folder_size_start": "fs_folder_size_start",
  "fs.recursive_search": "fs_recursive_search",
  "fs.recursive_search_start": "fs_recursive_search_start",
  "fs.content_search": "fs_content_search",
  "fs.content_search_start": "fs_content_search_start",
  "fs.compare_files": "fs_compare_files",
  "fs.watch_start": "fs_watch_start",
  "fs.watch_stop": "fs_watch_stop",
  "fileOperation.plan": "plan_file_operation",
  "fileOperation.start": "start_file_operation",
  "job.cancel": "cancel_job",
  "job.pause": "pause_job",
  "job.resume": "resume_job",
  "job.status": "get_job_status",
  "operationHistory.listRecent": "list_recent_operations",
  "operationHistory.clear": "clear_operation_history",
  "diagnostics.appDataHealth": "diagnostics_app_data_health",
  "diagnostics.exportBundle": "export_diagnostics_bundle",
  "preferences.get": "get_preferences",
  "preferences.set": "set_preference",
  "autostart.get": "get_autostart",
  "autostart.set": "set_autostart",
  "navigation.recordVisit": "navigation_record_visit",
  "navigation.listFavorites": "navigation_list_favorites",
  "navigation.addFavorite": "navigation_add_favorite",
  "navigation.removeFavorite": "navigation_remove_favorite",
  "navigation.renameFavorite": "navigation_rename_favorite",
  "navigation.listRecent": "navigation_list_recent",
  "navigation.listStarred": "navigation_list_starred",
  "navigation.toggleStarred": "navigation_toggle_starred",
  "navigation.isStarred": "navigation_is_starred",
  "navigation.clearRecent": "navigation_clear_recent",
  "navigation.removeRecent": "navigation_remove_recent",
  "network.profilesList": "network_profiles_list",
  "network.providersList": "network_providers_list",
  "network.profileAdd": "network_profile_add",
  "network.profileUpdate": "network_profile_update",
  "network.profileDelete": "network_profile_delete",
  "network.profileSetSecret": "network_profile_set_secret",
  "network.profileForgetFingerprint": "network_profile_forget_fingerprint",
  "network.connect": "network_connect",
  "network.disconnect": "network_disconnect",
  "network.connectionStatus": "network_connection_status",
  "network.profileTest": "network_profile_test",
  "network.discoverNeighborhood": "network_discover_neighborhood",
  "network.validateUri": "network_validate_uri",
  "terminal.spawn": "terminal_spawn",
  "terminal.write": "terminal_write",
  "terminal.resize": "terminal_resize",
  "terminal.kill": "terminal_kill",
  "terminal.capabilities": "terminal_capabilities",
  "terminal.profilesList": "terminal_profiles_list",
  "terminal.profileAdd": "terminal_profile_add",
  "terminal.profileUpdate": "terminal_profile_update",
  "terminal.profileDelete": "terminal_profile_delete",
  "terminal.profileSetDefault": "terminal_profile_set_default",
  "terminal.sessionsList": "terminal_sessions_list",
  "terminal.sendText": "terminal_send_text",
  "terminal.runCommand": "terminal_run_command",
  "terminal.spawnAndRun": "terminal_spawn_and_run",
  "plugin.list": "plugin_list",
  "plugin.install": "plugin_install",
  "plugin.uninstall": "plugin_uninstall",
  "plugin.toggle": "plugin_toggle",
  "fs.get_acl": "fs_get_acl",
  "fs.set_acl": "fs_set_acl",
  "fs.sync_directories": "fs_sync_directories",
};
